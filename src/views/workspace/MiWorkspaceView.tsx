...
import { obtenerMiWorkspacePaciente } from '../../api/WorkSpaceApi';

const MiWorkspaceView = () => {
  ...
  const { data: pacienteWorkspace, isLoading: isLoadingId, refetch: refetchId } = useQuery({
    queryKey: ['miWorkspaceIdAsignado'],
    queryFn: obtenerMiWorkspacePaciente,
    enabled: esPaciente,
  });

  useEffect(() => {
    if (esPaciente && pacienteWorkspace?.id) {
      setActiveWorkspaceId(pacienteWorkspace.id);
    } else if (route.params?.workspaceId) {
      setActiveWorkspaceId(route.params.workspaceId);
    }
  }, [pacienteWorkspace, route.params?.workspaceId, esPaciente]);

  const { data: workspace, isLoading: isLoadingDetalle, isError, refetch: refetchDetalle } = useQuery({
    queryKey: ['detalleWorkspace', activeWorkspaceId],
    queryFn: () => obtenerDetalleWorkspace({ id: activeWorkspaceId! }),
    enabled: !!activeWorkspaceId,
  });

  useFocusEffect(
    useCallback(() => {
      if (esPaciente) refetchId();
      if (activeWorkspaceId) refetchDetalle();
    }, [activeWorkspaceId, esPaciente])
  );
  ...
  const { mutate: removerNutricionista, isPending: isRemoving } = useMutation({
    ...
    onSuccess: (message) => {
      queryClient.invalidateQueries({ queryKey: ['detalleWorkspace', activeWorkspaceId] });
      setAlertConfig({
        visible: true,
  ...
  const handleExecuteDelete = () => {
    if (!selectedUser || !activeWorkspaceId) return;
    removerNutricionista({ workspace_id: activeWorkspaceId, user_id: selectedUser.id });
  };
  ...
  if (isLoadingId || (activeWorkspaceId && isLoadingDetalle)) {
    return (
      <LayoutHeader>
  ...
  if (isError || (!workspace && activeWorkspaceId)) {
    return (
      <LayoutHeader>
        <View style={styles.centerContainer}>
          <Text style={{ color: AppTheme.error, fontWeight: '600' }}>Error al obtener la información o área no asignada.</Text>
        </View>
      </LayoutHeader>
    );
  }
...
export default MiWorkspaceView;